    ...
    for (const auto& player : players) {
        ...
        if (dist < radius + 5) {
            ...
            if (!player.name.empty()) {
                m_tooltip.lines.push_back("Guild: " + player.name);
            }
...
void MapRenderer::renderBases(const std::vector<GuildBase>& bases,
    ...
    for (const auto& base : bases) {
        ...
        // Get guild color
        ImU32 baseColor;
        auto colorIt = guildColors.find(base.id);
        if (colorIt != guildColors.end()) {
            baseColor = ImGui::ColorConvertFloat4ToU32(colorIt->second);
        } else {
            baseColor = GUILD_COLORS[std::hash<std::string>{}(base.id) % 8];
        }
        ...
        if (dist < size + 5) {
            ...
            m_tooltip.lines.clear();
            m_tooltip.lines.push_back("Guild: " + base.name);
